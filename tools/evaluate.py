...
def evaluate_pop(
    ...
    sub.call(f"mkdir -p fitnessFiles/{run_directory}/{run_name}/", shell=True) # Creates a new directory to store fitness files
    ...
    logging.info(f"Writing {len(pop)} vxd files for xenobots: {n}")
    for n, individual in enumerate(pop):
        ...
        sub.call(f"cp id{n}.vxd fitnessFiles/{run_directory}/{run_name}/", shell=True)
