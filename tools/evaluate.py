...
def evaluate_pop(pop: List, run_directory: str, run_name: str, fitness_function: FitnessFunction):
    ...
    vxa = VXA() #TODO pass vxa tags in here INCREASE STEP TIME!
    ...
    sub.Popen(f"mkdir -p fitnessFiles/{run_directory}/{run_name}/", shell=True)
    ...
    sub.Popen("rm base.vxa", shell=True)
    ...
    for n, individual in enumerate(pop):
        ...
        sub.Popen(f"rm id{n}.vxd", shell=True)
    ...
    evaluation = sub.Popen(f"./voxcraft-sim -i fitnessFiles/{run_directory}/{run_name}/ -o fitnessFiles/{run_directory}/{run_name}/output.xml -f > fitnessFiles/{run_directory}/{run_name}/{run_name}.history", shell=True)
    
    evaluation.wait() #Waits for evaluation to finish
